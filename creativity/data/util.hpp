...
/** Returns a sample quantile from a pair of random access iterators over the range of sorted values
 * `[begin, end)`.  Linear interpolation is used for quantiles that lie between elements.  The
 * interpolation is such that the quantile for 0 returns the smallest value and the quantile for 1
 * returns the largest value.
 *
 * The range of values must be sorted in order for this function to work correctly--but this is not
 * checked.
 *
 * \param begin the random access iterator to the first value
 * \param end the past-the-end random access iterator
 * \param prob the probability for which the quantile should be calculated; must be between 0 and 1.
 * \returns NaN if `begin == end`, otherwise returns the requested quantile.
 */
