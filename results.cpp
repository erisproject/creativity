...
void readCSV(const std::string &filename) {
    ...
    // The data contains values like pre_whatever, piracy_whatever, public_whatever.  We need to convert those into three rows:
    // - one with whatever set to pre_whatever and piracy=public=0
    // - one with whatever set to piracy_whatever, piracy=1, public=0
    // - one with whatever set to public_whatever, piracy=0, public=1
    ...
    // pre_whatever, piracy_whatever, and public_whatever should all map to the same column, albeit
    ...
    for (auto &f : csv.fields()) {
        if (f[0] == '_') throw std::runtime_error("CSV file has invalid field `" + f + "': fields may not start with _");
        std::string data_field;
        if (f.substr(0, 4) == "pre_") data_field = f.substr(3);
        else if (f.substr(0, 7) == "piracy_") { data_field = f.substr(6); piracy_rows = true; }
        else if (f.substr(0, 7) == "public_") { data_field = f.substr(6); public_rows = true; }
    ...
    for (const auto &row : csv) {
        ...
        // First row: pre_ values
        for (unsigned fc = 0; fc < csv.fields().size(); fc++) {

            auto &d = csv.fields()[fc];
            if (d.substr(0, 7) != "piracy_" and d.substr(0, 7) != "public_") {
        ...
        if (piracy_rows) {
            for (unsigned fc = 0; fc < csv.fields().size(); fc++) {
                auto &d = csv.fields()[fc];
                if (d.substr(0, 4) != "pre_" and d.substr(0, 7) != "public_") {
                    rawdata(rows, data_column[d]) = row[fc];
                }
            }
        ...
        if (public_rows) {
            for (unsigned fc = 0; fc < csv.fields().size(); fc++) {
                auto &d = csv.fields()[fc];
                if (d.substr(0, 4) != "pre_" and d.substr(0, 7) != "piracy_") {
                    rawdata(rows, data_column[d]) = row[fc];
                }
            }
            if (piracy_rows) rawdata(rows, data_column["piracy"]) = 0;
    ...
    for (auto &dc : data_column) {
        if (dc.first.substr(0, 4) == "pre_" or dc.first.substr(0, 7) == "public_" or dc.first.substr(0, 7) == "piracy_")
            continue;
        std::string name(dc.first);
        if (dc.first[0] == '_') name = dc.first.substr(1);
